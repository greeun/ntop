...
pub fn render_process_list(f: &mut Frame, area: Rect, app: &App) {
    ...
    // Determine column widths based on available space
    let widths = if inner_width > 80 {
        vec![
    ...
    let rows: Vec<Row> = app
        ...
        .map(|(idx, (proc_info, depth))| {
            ...
            let name_display = format!(
                "{}{}",
                tree_prefix,
                if proc_info.framework.to_string() != "Generic" {
                    format!("{} ({})", proc_info.name, proc_info.framework)
                } else {
